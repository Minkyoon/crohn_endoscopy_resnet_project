...
model = torch.load('/home/minkyoon/crom/pyfile/relapse/resnet50_epoch100_trans2.pt')

new_model = torch.nn.Sequential(*(list(model.children())[:-1]))

# 새로운 모델의 출력 크기를 1024로 설정
new_model.add_module('fc', torch.nn.Linear(2048, 1024))
...
# CSV 파일이 있는 디렉토리 경로
csv_directory = "/home/minkyoon/crom/pyfile/relapse/mlp/"
    ...
    for filename in os.listdir(csv_directory):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                # feature 추출
                features = extract_features(model_image, custom_dataloader)
                ...
                writer.writerow({'pt_filepath': pt_filepath, 'label': label})
...
a=torch.load('/home/minkyoon/first/testclam/feature/unifeature/pt_files/1509252650.pt')
