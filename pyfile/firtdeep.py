...
def print_files_in_dir(root_dir, prefix):
    ...
    for i, file in enumerate(files):
      ##listdir 에서 파일 목록을 files로 가져옴!
      ## files 목록에는 0_CT_NonCOVID등등
      ## 각각 for문으로 동면서 file 
      ## 
        path = os.path.join(root_dir, file)
        file_list = os.listdir(path)
       ## 0_CT_Noncovid 에있는 파일하나 하나 확인
...
train_dataset_folder = str(root_dir + 'train')
...
torch.save(model_best, '/home/minkyoon/crom/pyfile/test_model.pt')
...
y_label = []
y_score = []
model = torch.load('/home/minkyoon/crom/pyfile/test_model.pt')
model.eval()
for i, (v_i, label) in enumerate(test_loader):
    # input data gpu에 올리기 
    v_i = v_i.float().to(device)
