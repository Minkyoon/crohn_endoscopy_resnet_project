...
torch.save(model_best, '/home/minkyoon/crom/pyfile/resnet50_anemia_epo100_real.pt')
import matplotlib.pyplot as plt
...
y_label = []
y_score = []
model = torch.load('/home/minkyoon/crom/pyfile/resnet50_anemia_epo100_.pt')
model.eval()
for i, (v_i, label) in enumerate(test_loader):
    # input data gpu에 올리기 
    v_i = v_i.float().to(device)
...
plt.savefig('confusition_anemia.png') # 이미지 저장
